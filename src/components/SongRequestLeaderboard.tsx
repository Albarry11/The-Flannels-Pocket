...
export const SongRequestLeaderboard: React.FC<SongRequestLeaderboardProps> = ({
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-0 relative">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto lg:overflow-y-hidden max-h-full pb-2 pt-0.5 lg:py-auto space-y-1.5 lg:space-y-0 lg:flex lg:overflow-x-auto lg:gap-4 items-center scrollbar-thin my-auto">
        {isLoading ? (
          ...
          <div className="w-full flex flex-col items-center justify-center p-6 sm:p-8 rounded-2xl sm:rounded-3xl bg-white/80 border border-sky-200/80 space-y-2 sm:space-y-3 max-w-md mx-auto text-center my-auto flex-shrink-0">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-amber-100 text-amber-600 mx-auto flex items-center justify-center">
              <Flame className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
            <h3 className="text-xs sm:text-sm font-extrabold text-[#0f2942]">Antrian Masih Kosong</h3>
            ...
            return (
              ...
              >
                  ...
                  <div>
                    ...
                    <div className="hidden lg:flex items-center justify-between gap-2 mb-1.5">
                    ...
                    <div className="flex items-center justify-between lg:justify-start gap-2 lg:gap-3">
                      <div className="flex items-center gap-2 lg:gap-3 min-w-0 flex-1">
                        {/* Mobile rank badge */}
                        <span className="lg:hidden font-mono font-black text-[10px] text-slate-500 bg-sky-50 px-1.5 py-0.2 rounded border border-sky-200 flex-shrink-0">
                          #{index + 1}
                        </span>
                        ...
                        <div className="relative flex-shrink-0 w-8 h-8 lg:w-16 lg:h-16 rounded-lg lg:rounded-2xl overflow-hidden shadow-xs lg:shadow-md border border-white bg-sky-100">
                          {req.artworkUrl ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-sky-600">
                              <Music className="w-4 h-4 lg:w-6 lg:h-6" />
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1">
                            <h4 className="text-[11px] lg:text-base font-extrabold text-[#0f2942] truncate tracking-tight leading-tight" title={req.title}>
                              {req.title}
                            </h4>
                            {isFulfilled ? (
                              <span className="text-[8px] font-bold px-1 rounded bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-0.5 flex-shrink-0 lg:hidden">
                                <CheckCircle2 className="w-2.5 h-2.5 text-emerald-600" /> Siap
                              </span>
                            ) : (
                              <span className="text-[8px] font-bold px-1 rounded bg-amber-100 text-amber-800 border border-amber-300 flex items-center gap-0.5 flex-shrink-0 lg:hidden">
                                <Clock className="w-2.5 h-2.5 text-amber-600" /> Antrian
                              </span>
                            )}
                          </div>
                          <p className="text-[9px] lg:text-xs text-sky-800 font-semibold truncate" title={req.artist}>
                            {req.artist} <span className="lg:hidden text-slate-500">• {req.requesterName}</span>
                          </p>
                          <p className="hidden lg:block text-[10px] text-slate-600 truncate mt-0.5">
                            diminta oleh <span className="font-bold text-sky-950">{req.requesterName}</span>
                          </p>
                        </div>
                      </div>

                      {/* Upvote Button (Mobile Row Inline) */}
                    ...
                    <div className="hidden lg:block mt-2 p-1.5 rounded-xl bg-sky-50/70 border border-sky-100 text-[11px] text-slate-600 space-y-0.5">
                  ...
                  <div className="hidden lg:flex pt-2.5 border-t border-sky-100 items-center justify-between gap-2 mt-2">
                    <button
                      onClick={() => handleUpvote(req.id)}
                      disabled={isFulfilled}
                  ...
                  {/* Mobile Notes / Subtitle (if available) */}
                  {req.notes && (
                    <div className="sm:hidden pt-0.5 border-t border-sky-100 text-[8px] text-slate-500 italic truncate" title={req.notes}>
                      "{req.notes}"
                    </div>
                  )}
                </div>
