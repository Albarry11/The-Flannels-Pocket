...
export const SongRequestLeaderboard: React.FC<SongRequestLeaderboardProps> = ({
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-0 relative">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto md:overflow-y-hidden pb-2 pt-0.5 space-y-1.5 md:space-y-0 md:flex md:overflow-x-auto md:gap-4 items-center scrollbar-thin my-auto">
        {isLoading ? (
          ...
          requests.map((req, index) => {
            ...
            return (
              <div
                key={req.id}
                className={`w-full md:w-[340px] flex-shrink-0 p-1.5 md:p-3.5 rounded-xl md:rounded-3xl border transition-all flex flex-col justify-between gap-1 md:gap-2.5 shadow-xs md:shadow-sm relative ${
                  isFulfilled
              ...
              >
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between md:justify-start gap-2 md:gap-3">
                      <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                        ...
                        {/* Artwork */}
                        <div className="w-8 h-8 md:w-14 md:h-14 rounded-lg md:rounded-2xl overflow-hidden shadow-xs border border-white bg-sky-100 flex-shrink-0">
                          {req.artworkUrl ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1">
                            <h4 className="text-[11px] md:text-sm font-extrabold text-[#0f2942] truncate tracking-tight leading-tight" title={req.title}>
                              {req.title}
                            </h4>
                            {isFulfilled ? (
                  ...
                  {req.notes && (
                    <div className="md:hidden pt-0.5 border-t border-sky-100 text-[8px] text-slate-500 italic truncate" title={req.notes}>
                      "{req.notes}"
                    </div>
                  )}
                </div>
